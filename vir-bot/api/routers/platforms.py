...
@router.get("/")
async def list_platforms(state: AppState = Depends(get_app_state)):
    result = []
...
async def start_platform(name: str, state: AppState = Depends(get_app_state)):
...
async def stop_platform(name: str, state: AppState = Depends(get_app_state)):
    if name not in state.adapters:
        return {"status": "error", "message": f"未知平台: {name}"}
